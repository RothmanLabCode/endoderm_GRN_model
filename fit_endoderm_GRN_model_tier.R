# "Modified Gauss-Newton algorithm" to fit endoderm GRN Tier model to transcriptomics data
...
bestParams <- c(
  3.278295e-02, #delta, Native degradation rate
  4.666656e-03, #a1, SKN-1 activation of med-2
  3.789606e-02, #a2, SKN-1 activation of med-1
  2.546014e-02, #a3, SKN-1 activation of end-3
  7.088873e-18, #a4, SKN-1 activation of end-1
  3.473743e-02, #a6, MED-2 activation of end-3
  4.436445e-18, #a17, MED-2 activation of end-1
  2.181668e-17, #a7, MED-1 activation of end-3
  2.048436e-01, #a8, MED-1 activation of end-1
  7.410022e-02, #a9, POP-1 activation of end-3
  8.742976e-17, #a10, POP-1 activation of end-1
  1.074840e-01, #a12, END-3 activation of elt-7
  1.559075e-17, #a13, END-1 activation of elt-7
  2.219584e-18, #a14, END-3 activation of elt-2
  3.344716e-02, #a15, END-1 activation of elt-2
  1.185834e-01, #f1, ELT-7 feedback
  3.839391e-01, #f2, ELT-2 feedback
  1.216969e+02, #theta1, Threshold required for ELT-7 feedback (ppm)
  2.141869e+01, #theta2, Threshold required for ELT-2 feedback (ppm)
  5.184828e+01 #tau, Transcription-activation delay (min)
)
...
#  a16 <- testParams[14]
  f1 <- testParams[16]
  f2 <- testParams[17]
  theta1 <- testParams[18]
  theta2 <- testParams[19]
  tau <- testParams[20]
  ...
  for(i in 1:(nTimepoints-1)){
    ...
    dmed1 <- (skn1*a2 - delta*med1_i)*!med1ko
    dend3 <- (skn1*a3 + med1_prev*a7 + med2_prev*a6 + pop1*a9 - delta*end3_i)*!end3ko
    dend1 <- (skn1*a4 + med2_prev*a17 + med1_prev*a8 + pop1*a10 - delta*end1_i)*!end1ko
    delt7 <- (end1_prev*a13 + end3_prev*a12 + f1*(elt7_prev>theta1)*elt7_prev - delta*elt7_i)*!elt7ko
    delt2 <- (end1_prev*a15 + end3_prev*a14 + f2*(elt2_prev>theta2)*elt2_prev - delta*elt2_i)*!elt2ko
