# "Modified Gauss-Newton algorithm" to fit endoderm GRN model to transcriptomics data
...
#   1. Comment out the line setting the value of leastSquares to Infinite (line 22).
#   2. Comment out the line overwriting bestParamGuesses with paramNames and bestParams (line 81).

#Set least squares value to Infinite if this is the first time running the program in this workspace
#Otherwise comment this line out
leastSquares <- Inf

# Settings for evolution (fitting) of model parameters
nGen <- 100000 #number of generations (cycles)
mu <- 0.1 #probability that a parameter gets mutated to a new value
...
paramNames <- c("delta", #Native degradation rate
                ...
                "a14", #END-1 activation of elt-2
                "a15", #ELT-7 activation of elt-2
                "f1", #ELT-7 feedback
                "f2", #ELT-2 feedback
...
bestParams <- c(6.495486e-02, #Native degradation rate
                0.03355822, #SKN-1 activation of med-2
                1.133834e-02, #SKN-1 activation of med-1
                9.410882e-04, #SKN-1 activation of end-3
                3.349926e-03, #SKN-1 activation of end-1
                1.131746e-02, #MED-2 activation of med-1
                3.851698e-02, #MED-2 activation of end-3
                1.668760e-01, #MED-1 activation of end-3
                4.066604e-02, #MED-1 activation of end-1
                1.807724e-14, #POP-1 activation of end-3
                2.172160e-01, #POP-1 activation of end-1
                2.046978e-01, #END-3 activation of end-1
                9.070484e-02, #END-3 activation of elt-7
                4.421926e-02, #END-1 activation of elt-7
                3.277627e-02, #END-1 activation of elt-2
                4.232214e-02, #ELT-7 activation of elt-2
                2.227494e-02, #ELT-7 feedback
                1.944363e-01, #ELT-2 feedback
                4.422987e+02, #Threshold required for ELT-7 feedback (ppm)
                3.661148e+00, #Threshold required for ELT-2 feedback (ppm)
                3.314187e+01) #Transcription-activation delay (min)

#If this line is commented out, parameter guesses will be drawn from bestParamGuesses data frame in the workspace.
#If this is the first time running the program or you manually adjusted the parameter values in bestParams, leave this line in.
#If you are iterating on the values currently stored in bestParamGuesses, comment this line out.
...
for(gen in 1:nGen){
  ...
  newParams <- rnorm(nParams, mean = 1, sd = mutationSD)*testParams # new values to be assigned to parameters for which mutation is TRUE
  ...
  f1 <- testParams[17]
  f2 <- testParams[18]
  theta1 <- testParams[19]
  theta2 <- testParams[20]
  tau <- testParams[21]
